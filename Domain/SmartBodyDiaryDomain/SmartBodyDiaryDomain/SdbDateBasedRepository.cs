...
public class SdbDateBasedRepository<TDataType> where TDataType : IDateRecord, new()
{
    ...
    public void AddOrUpdate(DateOnly date, TDataType record)
        => _data.AddOrUpdate(date, record, (_, _) => record);
    ...
    {
        return _data.TryRemove(day, out var result) ? result : default(TDataType);
    }
